...
interface Transform2DProps {
  initialPoints?: Point[];
  initialTransformedPoints?: Point[];
  onPointsUpdate?: (points: Point[], transformedPoints: Point[]) => void;
}

const Transform2D: React.FC<Transform2DProps> = ({
  initialPoints = [],
  ...
  const [transformedPoints, setTransformedPoints] = useState<Point[]>(initialTransformedPoints);
  ...
  useEffect(() => {
    if (onPointsUpdate) {
      onPointsUpdate(points, transformedPoints);
    }
  }, [points, transformedPoints, onPointsUpdate]);
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            <Box sx={{ textAlign: 'center', mb: 2 }}>
              ...
              (Check the results in this canvas)
            </Box>
            <Box 
              sx={{ 
                position: 'relative',
